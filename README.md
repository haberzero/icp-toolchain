...
4. 运行主命令行工具
        ...
        poetry run python ./src_main/main_cmd.py
...
作者本职工作为电子工程及嵌入式C，开发工作全凭个人热情以及下班后的休息时间。如在仓库结构/代码结构/自动化工具使用/文档 等层面出现疏漏或错误，还请多包容并礼貌指出，会尽快处理。

工程中的大部分代码都直接或间接地由AI参与生成，一切以功能的快速实现为重，辅以定期review及重构。但是请贡献者避免提交纯粹的vibe代码，vibe的前提是你清楚地知道自己在写什么。

现demo阶段代码结构本身并不保证绝对稳定，作者本人是初次尝试独自主导这种规模的项目，存在很多初期经验不足带来的错误设计（如错误处理的相关逻辑仍十分混乱）。但在当前阶段，这些非主要问题被刻意暂时忽略。

因此，直到ICP的稳定版出现前，初学者都切勿直接以当前代码作为学习范例。相关类似问题在此提前致歉。

此为长期项目，作者精力有限且必须长期稳定地把握主要开发方向和目标，暂无力顾及过多与未来主线无关的“愿望功能清单”。
