    ...
    def _tokenize_line_with_refs(self, content_line: str) -> None:
        ...
        while i < n:
            ...
            if char == '$':
                i += 1
                start = i
                # 收集符号引用内容：直到遇到空白、保留符号（包括$）为止，允许包含'.'
    ...
    def _tokenize_text_part(self, text: str):
        ...
        while i < n:
            ...
            if char == '(':
                ...
                # 所有的$符号应该由_tokenize_line_with_refs处理
                raise RuntimeError(
                    f"Internal error at line {self.line_num}: "
                    f"Unexpected $ symbol in _tokenize_text_part. "
                    f"This indicates a bug in the lexer logic. Please contact the developer."
                )
            else:
