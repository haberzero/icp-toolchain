...
统一管理所有路径构建逻辑，提供易用的路径访问接口。

本工程中存在三种路径体系：

1. icp_json_file_path（目标工作区项目结构路径）
    - 相对路径，记录在proj_root_dict中，用于描述目标工程的源代码结构
    - 格式：使用'/'作为分隔符，如 "src/ball/ball_entity"
    - 这是工程专有的命名约定，不是实际文件系统路径

2. icp-toolchain内部路径（工具链资源路径）
    - 访问icp-toolchain工具链自身的资源文件
    - 包括：系统提示词、用户提示词模板、应用数据等
    - 这些路径相对于src_main目录

3. 目标工程路径（用户项目路径）
    - 访问用户目标工程中的文件
    - 包括：项目数据、IBC代码、中间产物、配置文件等
    - 这些路径相对于用户工程根目录(work_dir_path)
"""
