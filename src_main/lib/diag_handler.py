    ...
    def _ensure_line_exists(self, line_num: int) -> None:
        if line_num not in self.diag_table:
            self.diag_table[line_num] = {
                'McbcEType': None,      # 错误信息类型
                'McbcWType': None,      # 警告信息类型
            }

    def set_line_error(self, line_num: int, error_type: McbcEType) -> None:
        if not isinstance(error_type, McbcEType):
            raise ValueError(f"Invalid error_type: {error_type}. Must be an instance of McbcEType.")
        self._ensure_line_exists(line_num)
        self.diag_table[line_num]['error_type'] = error_type

    def set_line_warning(self, line_num: int, warning_type: McbcWType) -> None:
        if not isinstance(warning_type, McbcWType):
            raise ValueError(f"Invalid warning_type: {warning_type}. Must be an instance of McbcWType.")
        self._ensure_line_exists(line_num)
