...
除了 `self` 这个关键字，其他所有IBC的关键字只在行开头被认为是关键字，其它地方出现的关键字会被当做普通文本处理。
...
- 构造函数：`__init__` 函数（推荐）或 与类同名的函数
- 构造函数参数必须有描述：为每个参数添加清晰的描述，以便生成目标代码时能够正确实例化
- 函数参数描述后也必须换行后再声明新参数
...
class UserManager($BaseManager: 使用公共基类管理生命周期):
    ...
    public
    description: 初始化用户管理器
    func __init__(
        数据库连接: 数据库连接对象，类型为 $db.Connection,
        配置: 用户管理配置字典):
        self.数据库 = 数据库连接
        self.配置 = 配置
        self.users = 创建空字典()
...
- 与 `description` 同时出现时，`@` 必须在 description和被修饰目标之间，独占一行
- 必须单行简短描述，不允许换行
- 提供设计决策、性能考虑、约束条件等
- 这些信息会在代码生成时提供给AI
...
- 用空格或符号分隔，点号 `.` 是引用的一部分
- 括号 `()` 不是引用的一部分
- **禁止猜测依赖符号的实现**：如果提供了依赖文件的IBC代码，必须按照实际IBC代码的定义进行引用，不允许自行假设或猜测类的构造函数参数、函数的参数列表或行为逻辑
...
- 同一目标多个 `@`
- 冒号后换行不缩进
- 类定义中同时存在与类同名的构造函数和 `__init__` 函数
