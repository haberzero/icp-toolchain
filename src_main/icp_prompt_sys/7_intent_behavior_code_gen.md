...
- 可见性关键字**仅允许在类定义内部使用**，不允许在顶层使用
- 关键字后**必须加冒号**（冒号不是关键字的一部分）
- 关键字行独立占据一行，**不需要后续行增加缩进**
- 关键字行只起标记作用，不生成具体节点
- 在出现可见性关键字后，**后续所有会形成符号的AST节点**（函数、变量）都会被打上对应的可见性标记
- 顶层类自身必须是 `public`，因此顶层无需使用可见性关键字
- 类内成员默认为 `public` 可见性
...
class DataProcessor():
    ...
    func process_data(输入数据):
        ...
        如果 验证结果:
            处理后数据 = self._process_internal(输入数据)
            返回 处理后数据
        否则:
            返回 None
...
- 可见性关键字不会影响缩进结构，后续内容保持与类体相同的缩进级别
- 可以在类内多次切换可见性，每次声明后的成员都使用新的可见性
- 如果类开始没有声明可见性，默认为 `public`
